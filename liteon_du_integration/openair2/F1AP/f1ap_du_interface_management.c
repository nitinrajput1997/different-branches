...
int DU_send_F1_SETUP_REQUEST(sctp_assoc_t assoc_id, f1ap_setup_req_t *setup_req)
{
  ...
  ie2->value.choice.RRC_Version.latest_RRC_Version.buf[0] = setup_req->rrc_ver;
  ...
  INT24_TO_OCTET_STRING(setup_req->rrc_ver_en, &rrcv_ext->extensionValue.choice.OCTET_STRING_SIZE_3_);
  ...
  for (int i=0; i < in->protocolIEs.list.count; i++) {
    ...
    switch (ie->id) {
      ...
      case F1AP_ProtocolIE_ID_id_GNB_CU_RRC_Version:
        AssertFatal(ie->criticality == F1AP_Criticality_reject,
                    "ie->criticality != F1AP_Criticality_reject\n");
        AssertFatal(ie->value.present == F1AP_F1SetupRequestIEs__value_PR_RRC_Version,
                    "ie->value.present != F1AP_F1SetupRequestIEs__value_PR_RRC_Version\n");
        resp.rrc_ver = BIT_STRING_to_uint8(&ie->value.choice.RRC_Version.latest_RRC_Version);
        if(ie->value.choice.RRC_Version.iE_Extensions) {
          ...
          if(ext->list.count > 0){
            ...
            OCTET_STRING_TO_INT24(&rrcext->extensionValue.choice.OCTET_STRING_SIZE_3_, resp.rrc_ver_en);
          }
        }
        break;
