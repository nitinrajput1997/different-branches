...
int CU_handle_F1_SETUP_REQUEST(instance_t instance, sctp_assoc_t assoc_id, uint32_t stream, F1AP_F1AP_PDU_t *pdu)
{
  ...
  req->rrc_ver = BIT_STRING_to_uint8(&ie->value.choice.RRC_Version.latest_RRC_Version);
        if(ie->value.choice.RRC_Version.iE_Extensions) {
          F1AP_ProtocolExtensionContainer_10696P228_t *ext = (F1AP_ProtocolExtensionContainer_10696P228_t *)ie->value.choice.RRC_Version.iE_Extensions;
          F1AP_RRC_Version_ExtIEs_t *rrcext;
          if(ext->list.count > 0){
            rrcext = ext->list.array[0];
            OCTET_STRING_TO_INT24(&rrcext->extensionValue.choice.OCTET_STRING_SIZE_3_, req->rrc_ver_en);
          }
        }
...
int CU_send_F1_SETUP_RESPONSE(sctp_assoc_t assoc_id, f1ap_setup_resp_t *f1ap_setup_resp)
{
  ...
  ie4->value.choice.RRC_Version.latest_RRC_Version.buf = calloc(1, sizeof(char));
  ie4->value.choice.RRC_Version.latest_RRC_Version.buf[0] = f1ap_setup_resp->rrc_ver;
  ie4->value.choice.RRC_Version.latest_RRC_Version.size = 1;
  ie4->value.choice.RRC_Version.latest_RRC_Version.bits_unused = 5;
  ...
  rrcv_ext->criticality = F1AP_Criticality_reject; 
  rrcv_ext->extensionValue.present = F1AP_RRC_Version_ExtIEs__extensionValue_PR_OCTET_STRING_SIZE_3_;
  INT24_TO_OCTET_STRING(f1ap_setup_resp->rrc_ver_en, &rrcv_ext->extensionValue.choice.OCTET_STRING_SIZE_3_);
